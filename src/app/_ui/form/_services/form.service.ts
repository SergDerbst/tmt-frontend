...
export class FormService {
    ...
    assemble(config: FormConfig):FormGroup {
        ...
        group['submitService'] = config.submitService;
    ...
    private formControlGroup(elements: FormElementBase<string>[]) {
        ...
        if (elements !== undefined) {
            for (let i = 0, len = elements.length; i < len; i++) {
                ...
                if (group[elements[i].key].items && group[elements[i].key].items.length > 0) {
                    group[elements[i].key].setValue(group[elements[i].key].items[0].value);
                }
            }
        }
